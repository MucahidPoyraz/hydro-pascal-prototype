/**
 * HPL Cylinder Calculator — vanilla JS (English strings)
 * English mirror of calculation-program.js. Same logic/formulas, only the
 * user-facing text differs, so the EN page behaves exactly like the TR page.
 */
(function () {
  'use strict';

  // ---- Constants (identical to the TR version's $rodMap / $custom_strokes) ----
  var rodMap = {
    25: [16],
    30: [25],
    32: [20],
  ...
  var PLACEHOLDER_IMG =
    ...
    encodeURIComponent(
      ...
      '<text x="50%" y="50%" fill="#64748b" font-family="Arial" font-size="20" text-anchor="middle" dominant-baseline="middle">Technical drawing not found</text>' +
      '</svg>'
    );
  ...
  function init() {
    ...
    if (!form) return; // no calculator on this page
  ...
  function onBoreChange() {
    ...
    rodSel.innerHTML = '<option value="">Select...</option>';
  ...
  // ---------------------------------------------------------------------
  // Calculation (identical formulas to the TR version)
  // ---------------------------------------------------------------------
  function onSubmit(e) {
    e.preventDefault();
    ...
    if (!(pPress > 0) || !(pFlow > 0) || !(cDiam > 0) || !(stroke > 0) || !(rDiam > 0)) {
      showFormNotice('Please fill in all fields.');
      return;
    }
    hideFormNotice();
  ...
  function renderResults(r) {
    ...
    grid.innerHTML = [
      resItem('Extension Force', fmt(r.extF) + ' ' + r.unitLabel),
      resItem('Retraction Force', fmt(r.retF) + ' ' + r.unitLabel),
      resItem('Oil Volume', fmt(r.vTotal) + ' L'),
      resItem('Cycle Time', fmt(r.tTotal) + ' s'),
      resItem('Drive Power', fmt(r.kw) + ' kW'),
      resItem('Power (HP)', fmt(r.hp) + ' HP')
    ].join('');
  }
  ...
  function renderMatchingProducts(cDiam, rDiam, stroke) {
    ...
    if (found.length === 0) {
      box.innerHTML =
        '<div class="hpl-notice hpl-notice-warn">No ready-made product found for this size (Ø' + cDiam + ' / Ø' + rDiam + ' / ' + stroke + 'mm stroke). Please contact us for custom manufacturing.</div>';
      return;
    }
    ...
    return (
      '<div class="hpl-accordion closed">' +
        '<button type="button" class="hpl-accordion-header" onclick="this.parentElement.classList.toggle(\'closed\')">' +
          '<span><strong>SERIES:</strong> ' + escapeHtml(seri) + ' &nbsp;|&nbsp; <strong>CODE:</strong> <span class="hpl-accordion-code">' + escapeHtml(code) + '</span></span>' +
          '<span class="hpl-accordion-icon">▲</span>' +
        '</button>' +
        '<div class="hpl-accordion-content">' +
          '<div class="hpl-drawing"><img src="' + imgSrc + '" alt="' + escapeHtml(seri) + ' technical drawing" loading="lazy" onerror="this.onerror=null;this.src=\'' + PLACEHOLDER_IMG + '\';"></div>' +
